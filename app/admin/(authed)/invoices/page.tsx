...
function statusOf(inv: InvoiceListItem): { label: string; cls: string } {
  if (inv.paid_at) return { label: "Paid", cls: "bg-emerald-500/15 text-emerald-300" };
  if (inv.sent_at) return { label: "Sent", cls: "bg-sky-500/15 text-sky-300" };
  return { label: "Draft", cls: "bg-white/10 text-white/50" };
}
...
export default async function InvoicesPage({
  ...
  return (
    <div className="pb-24 lg:pb-0">
      ...
      {msg && (
        <div className="mb-6 border border-[#F96302]/40 bg-[#F96302]/10 px-4 py-3 text-sm text-[#F9A968]">
          {msg}
        </div>
      )}

      {invoices.length === 0 ? (
        <div className="border border-white/10 bg-black/30 px-6 py-16 text-center">
          <p className="text-base text-white/60">No invoices yet.</p>
          <Link
            href="/admin/invoices/new"
        ...
        <div className="overflow-x-auto border border-white/10">
          <table className="w-full min-w-[640px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-white/10 bg-white/[0.03] text-left text-[11px] uppercase tracking-[0.1em] text-white/40">
              ...
              {invoices.map((inv) => {
                ...
                return (
                  <tr key={inv.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                    <td className="px-4 py-3 font-bold text-white">#{inv.id}</td>
                    <td className="px-4 py-3 text-white/80">{inv.customer_name ?? "—"}</td>
                    ...
                    <td className="px-4 py-3 text-white/50">{fmtDate(inv.created_at)}</td>
                    <td className="px-4 py-3">
                      <a
